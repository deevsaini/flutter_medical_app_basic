...
class _PConfirmAppointmentScreenState extends State<PConfirmAppointmentScreen> {
  ...
  void init() async {
    ...
    await getConfiguration().catchError((e) {
      log(e.toString());
    });
    getDoctor();
  }
    ...
    appointmentRequest.addAppointment(req).then((value) async {
      if (appStore.isBookedFromDashboard) {
        ...
        value?.woocommerce_redirect != null
            ? await WebViewPaymentScreen(checkoutUrl: value?.woocommerce_redirect.validate())
                .launch(context)
                .then((value) => toast(value?.message.validate() ?? 'Appointment Booked Successfully'))
            : test();
      } else {
        void test() {
          finish(context);
          ...
          toast(value?.message.validate() ?? languageTranslate('lblAppointmentBookedSuccessfully'));
        ...
        value?.woocommerce_redirect != null
            ? WebViewPaymentScreen(
              ...
              ).launch(context).then((value) => toast(value?.message.validate() ?? languageTranslate('lblAppointmentBookedSuccessfully')))
            : test();
      }
    }).catchError((e) {
      appStore.setLoading(false);
  ...
  Widget confirmedAppointment() {
    return Column(
      ...
      children: [
        ...
        30.height,
        Text(languageTranslate('lblAppointmentIsConfirmed'), style: primaryTextStyle(size: 24), textAlign: TextAlign.center),
        20.height,
        Align(
          alignment: Alignment.bottomCenter,
          child: Text(languageTranslate('lblThanksForBooking'), style: secondaryTextStyle()),
        ),
      ],
    );
  }
    ...
    return SizedBox(
      ...
      child: Column(
        ...
        children: [
          ...
          30.height,
          Text(
            "${getStringAsync(FIRST_NAME)}, ${languageTranslate("lblAppointmentConfirmation")}",
          ...
          isProEnabled()
              ? Text(
                  '${languageTranslate('lblClinic')}: ${appointmentAppStore.mClinicSelected?.clinic_name.validate()} ',
          ...
          if (appointmentAppStore.mDescription.validate().isNotEmpty)
            Text('${appointmentAppStore.mDescription}',
                style: primaryTextStyle(), textAlign: TextAlign.center, maxLines: 10, overflow: TextOverflow.ellipsis),
          8.height,
          Observer(
            builder: (_) => Align(
              alignment: Alignment.bottomCenter,
              child: AppButton(
                text: languageTranslate('lblConfirmAppointment'),
